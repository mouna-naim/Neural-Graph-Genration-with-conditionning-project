...
from autoencoder import VariationalAutoEncoder, AutoEncoder, ConditionalAutoEncoder
...
autoencoder = ConditionalAutoEncoder(args.spectral_emb_dim+1, args.hidden_dim_encoder, args.hidden_dim_decoder, args.latent_dim, args.n_layers_encoder, args.n_layers_decoder, args.n_max_nodes,args.n_condition).to(device)
...
if args.train_autoencoder:
    ...
    for epoch in range(1, args.epochs_autoencoder+1):
        ...
        scheduler.step()
        #warmup_scheduler.step(val_loss_avg)
...
if args.train_denoiser:
    ...
    for epoch in range(1, args.epochs_denoise+1):
        ...
        for data in train_loader:
            ...
            loss = p_losses(denoise_model, x_g, t, data.stats, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, loss_type="huber",bert_emb=data.embedding)
            loss.backward()
...
with open("output.csv", "w", newline="") as csvfile:
    ...
    for k, data in enumerate(tqdm(test_loader, desc='Processing test set',)):
        ...
        samples = sample(denoise_model, data.stats, latent_dim=args.latent_dim, timesteps=args.timesteps, betas=betas, batch_size=bs,bert_emb=data.embedding)
        x_sample = samples[-1]
