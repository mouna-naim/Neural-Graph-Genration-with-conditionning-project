...
from extract_feats import extract_feats, extract_numbers, extract_numbers_with_bert
    ...
    if dataset == 'test':
        ...
        if os.path.isfile(filename):
            ...
            for line in fr:
                ...
                desc = tokens[1:]
                desc = "".join(desc)
                feats_stats,embedding = extract_numbers_with_bert(desc)
                feats_stats = torch.FloatTensor(feats_stats).unsqueeze(0)
                data_lst.append(Data(stats=feats_stats, filename = graph_id,embedding=embedding ))
            fr.close()                    
        ...
        if os.path.isfile(filename):
            ...
            for fileread in tqdm(files):
                ...
                feats_stats,embedding = extract_feats(fstats)
                feats_stats = torch.FloatTensor(feats_stats).unsqueeze(0)

                data_lst.append(Data(x=x, edge_index=edge_index, A=adj, stats=feats_stats, filename = filen,embedding=embedding))
            torch.save(data_lst, filename)
            print(f'Dataset {filename} saved')
    return data_lst
